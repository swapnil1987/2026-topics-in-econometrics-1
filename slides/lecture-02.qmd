...
::: {.key-idea}
Every OLS line with an intercept passes through **$(\bar X,\bar Y)$**.
:::
...
## With one regressor, R-squared is squared correlation

Since $\hat Y_i-\bar Y=\hat\beta_1(X_i-\bar X)$,

$$
R^2=\frac{\hat\beta_1^2S_{XX}}{S_{YY}}
=\frac{S_{XY}^2}{S_{XX}S_{YY}}=r_{XY}^2.
$$

- This identity holds for simple OLS **with an intercept**.
- Squaring removes the sign: equal positive and negative correlations give the same $R^2$.
- $R^2$ describes strength of linear fit, not the direction of the association.

::: {.notes}
S_YY denotes TSS. Both sample variances must be positive for the correlation.
For multiple regressors, the R-squared is not the squared correlation of
Y with any one regressor. A zero sample linear correlation does not rule
out a nonlinear relationship.
:::
